...
int selectMovement(char movementChoice){
  ...
  if(movementChoice == 'w'){
    clear;
    // La spiegazione dell'if la trovi quando ti sposti a destra(più sotto qui nel code)
    if(player.posY > 0 && disegnoCitta[cittaAttuale][player.posY-1][player.posX] != 1)
      player.posY--;
  }
  // DESTRA
  else if(movementChoice == 'd'){
    clear;
    // Se a destra hai spazio vai, e se a destra non hai un blocco verde( != 1 --> blocco verde(1))
    if(player.posX < CITTA_COLONNE - 1 && disegnoCitta[cittaAttuale][player.posY][player.posX+1] != 1) 
      player.posX++;
  }
  else if(movementChoice == 'a'){
    clear;
    if(player.posX > 0 && disegnoCitta[cittaAttuale][player.posY][player.posX-1] != 1)
      player.posX--;
  }
  else if(movementChoice == 's'){
    clear;
      if(player.posY < CITTA_RIGHE - 1 && disegnoCitta[cittaAttuale][player.posY+1][player.posX] != 1)
      player.posY++;
  }
...
//-------------------------------------------------------------------------------------------------------------------------------------------//
    ...
    // POKEMON //////////////////////
    clear;
      // hai il 30% di incontrare un pokemon, e se il pokemon in slot 0 ha più di 0 di vita
